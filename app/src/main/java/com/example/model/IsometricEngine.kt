...
data class Line3D(
    ...
    val p1: Point3D get() = start
    val p2: Point3D get() = end
}
...
object IsometricEquipmentMeshGenerator {
    fun generateMesh(type: IsometricModelType): IsoMesh {
        return IsometricEngine.buildModelMesh(type)
    }
}
...
object IsometricEngine {
    ...
    private fun buildMbbrTankMesh(): IsoMesh {
        ...
        faces.add(Face3D(listOf(t0, t1, t2, t3), ssColorTop))
        faces.add(Face3D(listOf(t3, t2, b2, b3), ssColorFront))
        faces.add(Face3D(listOf(t1, t0, b0, b1), ssColorSide))
        faces.add(Face3D(listOf(t0, t3, b3, b0), ssColorSide))
    ...
    // 2. SS Pressure Cyclone & Hopper Vessel
